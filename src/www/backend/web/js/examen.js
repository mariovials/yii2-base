$(function() {

let lista = $('#imagenes-examen')
let items = lista.find('.imagen')

btn = $('#btn-agregar-imagen')
let subiendo = 0

btn.on('click', function(e) {
  e.preventDefault()
  e.stopPropagation()

  url = $(this).attr('href')

  // crea el input
  var input = $(document.createElement('input'))
  input.attr("type", "file")
  input.attr("id", "input-agregar-paginas")
  input.attr("name", "ExamenImagen[imagen]")
  input.attr('accept', 'image/*')
  input.attr("multiple", "multiple")

  // cuando cambia el value del input
  // agrega los items al form para enviarlos
  // y los envia
  input.on('change', function() {
    $(this).simpleUpload(url, {
      start: function(file) {
        subiendo++
        this.item = $(`
          <div class="item imagen">
            <div class="informacion">
              <div class="nombre"> ${this.upload.file.name} </div>
            </div>
            <div class="miniatura">Subiendo...</div>
          </div>`)
        btn.before(this.item)
      },
      progress: function(progress) {
        console.log(progress)
      },
      success: function(data) {
        console.log(data);
        this.item.find('miniatura').html('Listo!')
      },
      error: function(error) {
        console.log(error)
      },
      complete: function() {
        subiendo--
      },
      finish: function() {
        if (subiendo == 0) {
          window.location.reload()
        }
      },
    })
  })

  input.trigger('click')
})

$('#examen-imagenes .item .opcion .btn-eliminar').on('click', function(e) {
  e.preventDefault()
  item = $(this).closest('.item')
  $.get($(this).attr('href'), function() {
    item.remove()
  })
})

})
